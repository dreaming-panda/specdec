...
class LLM:
    ...
    def layer_compute(self, 
        ...
        hidden_states = hidden_states.reshape(bsz, self.num_heads // self.world_size, q_len, -1)
        ...
        #     _, indices = torch.abs(neuron_stat).topk(k=int(0.5 * hidden_states.shape[-1]), largest=False, dim=-1)
